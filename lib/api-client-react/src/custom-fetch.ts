...
export async function customFetch<T = unknown>(
  ...
  // Inject auth token from Zustand persisted store
  function getStoredAuth(): { accessToken: string | null; refreshToken: string | null } {
    ...
    try {
      const stored = localStorage.getItem("promptvault-auth");
      if (stored) {
  ...
  // Auto-refresh access token on 401 if a refresh token is available
  if (response.status === 401 && refreshToken && !resolveUrl(input).includes("/auth/refresh")) {
    try {
      ...
      if (refreshRes.ok) {
        const refreshData = await refreshRes.json() as { accessToken?: string; refreshToken?: string; user?: unknown };
          ...
          try {
            ...
            if (stored) {
              ...
              if (refreshData.refreshToken) {
                parsed.state.refreshToken = refreshData.refreshToken;
              }
              localStorage.setItem("promptvault-auth", JSON.stringify(parsed));
            }
          } catch {
            // Ignore storage errors
          }
