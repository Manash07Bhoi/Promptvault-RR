...
export default function FollowersPage() {
  ...
  const filtered = search
    ? followers.filter((u: any) => u.displayName?.toLowerCase().includes(search.toLowerCase()) || u.username?.toLowerCase().includes(search.toLowerCase()))
    : followers;
    ...
    <PublicLayout>
      <div className="max-w-2xl mx-auto px-4 py-10">
        ...
        <div className="flex items-center gap-2 mb-6">
          ...
          <h1 className="text-xl font-display font-bold text-foreground">Followers</h1>
        </div>

        <div className="relative mb-6">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <Input className="pl-9" placeholder="Search followers..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
